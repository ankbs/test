<#
.SYNOPSIS
03-Run-Purview-Label-PriorityManager.ps1
Ziel: Labels (Name & DisplayName) anzeigen, Filterung, Selektion, neue Prioritäten zuweisen.
#>
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        Title="Purview Label Priority Manager" Height="700" Width="1000">
    <Grid Margin="10">
        <Grid.RowDefinitions>
        ...
        <DockPanel Grid.Row="0" LastChildFill="False" Margin="0,0,0,10">
            ...
            <TextBlock Text="Microsoft Purview Label Priority Manager" FontWeight="Bold" FontSize="20" VerticalAlignment="Center" />
        ...
        <Grid Grid.Row="1" Margin="0,0,0,10">
            ...
            <StackPanel Orientation="Horizontal" Grid.Row="0" Margin="0,0,0,5">
                <Label Content="Buchungskreis (Filter):" VerticalAlignment="Center"/>
                <TextBox Name="txtFilter" Width="100" Margin="5,0,10,0"/>
                <Button Name="btnLoadLabels" Content="Laden" Width="100" Margin="0,0,10,0"/>
            </StackPanel>
            <Grid Grid.Row="1" Margin="0,5,0,0">
                ...
                <ListBox Name="lstAllLabels" Grid.Column="0" SelectionMode="Extended" DisplayMemberPath="DisplayNameExtended"/>
                <StackPanel Grid.Column="1" Orientation="Vertical" Margin="10,0">
                    <Button Name="btnAdd" Content="→" Width="50" Margin="0,0,0,10"/>
                    <Button Name="btnRemove" Content="←" Width="50"/>
                </StackPanel>
                <ListBox Name="lstSelectedLabels" Grid.Column="2" SelectionMode="Extended" DisplayMemberPath="DisplayNameExtended"/>
            </Grid>
        </Grid>

        <!-- Buttons -->
        <StackPanel Grid.Row="2" Orientation="Horizontal" HorizontalAlignment="Right" Margin="0,10,0,0">
            <Button Name="btnSetPriorities" Content="Neue Prioritäten zuweisen" Width="200" Margin="0,0,10,0"/>
...
$btnLoadLabels = $window.FindName("btnLoadLabels")
$txtFilter = $window.FindName("txtFilter")
$lstAllLabels = $window.FindName("lstAllLabels")
$lstSelectedLabels = $window.FindName("lstSelectedLabels")
$btnAdd = $window.FindName("btnAdd")
$btnRemove = $window.FindName("btnRemove")
$btnSetPriorities = $window.FindName("btnSetPriorities")
...
if ($CompanyLogoBase64) {
    $logo = Create-ImageFromBase64 $CompanyLogoBase64
    ...
    $imgCompanyLogo.Source = $bmp
}
if ($ProductLogoBase64) {
    $logo2 = Create-ImageFromBase64 $ProductLogoBase64
    $bmp2 = New-Object System.Windows.Media.Imaging.BitmapImage
    $ms2 = New-Object System.IO.MemoryStream
    $logo2.Save($ms2, [System.Drawing.Imaging.ImageFormat]::Png)
    $ms2.Position = 0
    $bmp2.BeginInit(); $bmp2.StreamSource = $ms2; $bmp2.CacheOption = "OnLoad"; $bmp2.EndInit()
    $imgProductLogo.Source = $bmp2
}

$btnLoadLabels.Add_Click({
    try {
        $filter = $txtFilter.Text.Trim()
        $labels = Get-Label | Where-Object {
            if ($filter -match '^\d{4}$') {
                $_.Name -match "^$filter\b" -or $_.Name -match "\b$filter$"
            } else {
                $true
            }
        }
...
$btnAdd.Add_Click({
    foreach ($item in $lstAllLabels.SelectedItems) { $lstSelectedLabels.Items.Add($item) }
})
$btnRemove.Add_Click({
    foreach ($item in $lstSelectedLabels.SelectedItems) { $lstSelectedLabels.Items.Remove($item) }
})

$btnSetPriorities.Add_Click({
    $selected = $lstSelectedLabels.Items
    if ($selected.Count -eq 0) {
        [System.Windows.MessageBox]::Show("⚠️ Bitte Labels auswählen!", "Keine Auswahl", "OK", "Warning")
        return
    }
    ...
    foreach ($label in $selected) {
        try {
            $allLabels = Get-Label
            $labelWithHighestPriority = $allLabels | Sort-Object Priority -Descending | Select-Object -First 1
            $newPriority = $labelWithHighestPriority.Priority

            $childs = $allLabels | Where-Object { $_.ParentId -eq $label.ExchangeObjectId }
    ...
    # Nach Abschluss: Parentlabel "Nur für internen Dienstgebrauch" setzen
    $allLabels = Get-Label
    $highestPrio = ($allLabels | Sort-Object Priority -Descending | Select-Object -First 1).Priority
    $parentInterner = $allLabels | Where-Object { $_.DisplayName -like "*Nur für internen Dienstgebrauch*" -and $_.ParentId -eq $null } | Select-Object -First 1
    if ($parentInterner) {
    ...
    # Danach Parentlabel "Vertraulich" setzen
    $allLabels = Get-Label
    $highestPrio = ($allLabels | Sort-Object Priority -Descending | Select-Object -First 1).Priority
    $parentVertraulich = $allLabels | Where-Object { $_.DisplayName -like "*Vertraulich*" -and $_.ParentId -eq $null } | Select-Object -First 1

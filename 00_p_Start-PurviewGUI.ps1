# === Parameter definieren ===
param (
    [string]$UserPrincipalName = "user@domain.tld",
    [string]$Tenantdomain      = "domain.tld",
    [string]$CompanyLogoPath = "",
    [string]$CompanyLogoUrl = "",
    [string]$CompanyLogoBase64 = "data:image/png;base64,", # Companylogo als eingebettetes Base64
    [string]$LogoUrl = "",
    [string]$ProductLogoBase64 = "data:image/png;base64,", # Produktlogo als eingebettetes Base64
    [string]$LogoGIFUrl = "",
    [string]$MailToPrimary = "michael.kirst-neshva@bdo-digital.eu",
    [string]$MailToSecondary = "",
    [string]$LogFolder = "C:\Temp\script\",
    [string]$MSPPartner = "Cloud Security && Compliance Services",
    [string]$MSPNameAP  = "Michael Kirst-Neshva",
    [string]$MSPMail    = "support@domainen.io",
    [string]$MSPURL     = "https://www.domainen.io"
)

if (!(Test-Path $LogFolder)) { New-Item -ItemType Directory -Path $LogFolder -Force | Out-Null }

# === Zeitstempel / Logdateien ===
$DatumJetzt = Get-Date -Format 'yyyyMMdd_HHmmss'
$DatumAnzeige = Get-Date -Format 'dd.MM.yyyy'
$cleanUser = $UserPrincipalName -replace '[^a-zA-Z0-9]', '_'
$LogFolder = $LogFolder.TrimEnd('\')
$LogFile = [System.IO.Path]::Combine($LogFolder, "Label_Status_LOG_$DatumJetzt.log")
$CreatedLabelsCsv = [System.IO.Path]::Combine($LogFolder, "Erstellte_Labels_$DatumJetzt.csv")
$StatusReportCsv = [System.IO.Path]::Combine($LogFolder, "Label_Status_$DatumJetzt.csv")

# === Konfigurationsdatei für GUI-Settings ===
$GuiConfigPath = Join-Path $LogFolder "GUIConfig.json"

# === Logging + Error-Handling ===
function Log {
    param([string]$Message, [string]$Level = "INFO")
    $timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    $prefix = switch ($Level) {
        "INFO"     { "ℹ️" }
        "SUCCESS"  { "✅" }
        "ERROR"    { "❌" }
        "DEBUG"    { "🐛" }
        default    { "🔹" }
    }
    $logEntry = "$timestamp $prefix $Message"
    Add-Content -Path $LogFile -Value $logEntry -Encoding utf8
    Write-Host $logEntry -Encoding utf8
}

function Handle-Error {
    param([string]$Message, [System.Exception]$ErrorObject)
    Log "$Message $($ErrorObject.Exception.Message)" "ERROR"
    exit 1
}

# === Zufalls-GIF fallback wenn kein GIF übergeben wurde
if (-not $LogoGIFUrl -or $LogoGIFUrl.Trim() -eq "") {
    $StandardGIFs = @(
        "https://media.tenor.com/I6kN-6X7nhAAAAAi/loading-buffering.gif",
        "https://media.tenor.com/On7kvXhzml4AAAAj/loading-gif.gif",
        "https://media.tenor.com/2uyENRmiUt0AAAAC/circle-loading.gif"
    )

    $LogoGIFUrl = Get-Random -InputObject $StandardGIFs
    Log "🎞️ Animiertes GIF für Splash: $LogoGIFUrl" "INFO"
    Write-Host "🔄 Kein GIF übergeben – zufälliges Standard-GIF ausgewählt: $LogoGIFUrl" -ForegroundColor Cyan
}

if ($LogoUrl -and $LogoUrl.Trim() -ne "") {
    Log "🏷️ Produktlogo wird geladen: $LogoUrl" "DEBUG"
}

# === Style-Konfiguration ===
$IconStyle = "Online"  # Standard: "Unicode", alternativ: "Online"

# === Symboldefinitionen ===
$SectionIcons = @{
    "MFA"      = @{ Unicode = "🔐"; Url = "https://cdn-icons-png.flaticon.com/512/565/565547.png" }
    "Email"    = @{ Unicode = "📧"; Url = "https://cdn-icons-png.flaticon.com/512/561/561127.png" }
    "Log"      = @{ Unicode = "📝"; Url = "https://cdn-icons-png.flaticon.com/512/833/833281.png" }
}

function Get-SectionIcon {
    param([string]$Key)
    if ($IconStyle -eq "Online") {
        try {
            $url = $SectionIcons[$Key].Url
            $wc = New-Object Net.WebClient
            $bytes = $wc.DownloadData($url)
            $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
            $img = [System.Drawing.Image]::FromStream($ms)
            Log "Online-Icon für $Key erfolgreich geladen." "SUCCESS"
            return $img
        }
        catch {
            Log "Fehler beim Laden des Online-Icons für $Key. Fallback auf Unicode." "ERROR"
            return $null
        }
    } else {
        return $SectionIcons[$Key].Unicode
    }
}

function Get-OnlineIcon {
    param (
        [string]$Url,
        [string]$Section = "unknown"
    )
    try {
        Log "🔄 [$Section] Versuche Online-Icon von $Url zu laden..." "INFO"
        Write-Host "🔄 [$Section] Versuche Online-Icon von $Url zu laden..."
        $response = Invoke-WebRequest -Uri $Url -UseBasicParsing -ErrorAction Stop
        if ($response.StatusCode -ne 200) {
            throw "HTTP $($response.StatusCode)"
        }
        $bytes = $response.Content
        $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
        $image = [System.Drawing.Image]::FromStream($ms)
        Log "✅ [$Section] Icon erfolgreich geladen." "SUCCESS"
        Write-Host "✅ [$Section] Icon erfolgreich geladen."
        return $image
    } catch {
        Log "❌ [$Section] Fehler beim Laden von ${Url}: $_" "ERROR"
        Write-Host "❌ [$Section] Fehler beim Laden von ${Url}: $_"
        return $null
    }
}

Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing

# === Vorbelegung aus GUIConfig.json (falls vorhanden)
if (Test-Path $GuiConfigPath) {
    try {
        $GuiConfig = Get-Content -Raw -Path $GuiConfigPath | ConvertFrom-Json
        if ($GuiConfig.UserPrincipalName) { $UserPrincipalName = $GuiConfig.UserPrincipalName }
        if ($GuiConfig.Tenantdomain)      { $Tenantdomain      = $GuiConfig.Tenantdomain }
        if ($GuiConfig.MailToPrimary)     { $MailToPrimary     = $GuiConfig.MailToPrimary }
        if ($GuiConfig.MailToSecondary)   { $MailToSecondary   = $GuiConfig.MailToSecondary }
        if ($GuiConfig.LogFolder)         { $LogFolder         = $GuiConfig.LogFolder }
        Log "📥 GUI-Konfiguration aus $GuiConfigPath geladen." "INFO"
    } catch {
        Log "⚠️ Fehler beim Einlesen der GUI-Konfiguration: $_" "ERROR"
    }
}

$form = New-Object Windows.Forms.Form
$form.Text = "Microsoft Purview – Startkonfiguration"
$form.Size = New-Object Drawing.Size(580, 640)
$form.StartPosition = "CenterScreen"
$form.Topmost = $true
$form.Font = New-Object Drawing.Font("Segoe UI Emoji", 9)

# === Company-Logo ===
if ($CompanyLogoBase64 -and $CompanyLogoBase64.Length -gt 100) {
    try {
        $clean = $CompanyLogoBase64 -replace '^data:image\/[a-z]+;base64,', ''
        $bytes = [Convert]::FromBase64String($clean)
        $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
        $img = [System.Drawing.Image]::FromStream($ms)
        $pic = New-Object Windows.Forms.PictureBox
        $pic.Image = $img
        $pic.SizeMode = "Zoom"
        $pic.Size = New-Object Drawing.Size(48, 48)
        $pic.Location = New-Object Drawing.Point(10, 10)
        $form.Controls.Add($pic)
    } catch {
        Log "❌ Fehler beim Laden des Companylogos: $_" "ERROR"
    }
}

# === Produktlogo ===
if ($ProductLogoBase64 -and $ProductLogoBase64.Length -gt 100) {
    try {
        $clean = $ProductLogoBase64 -replace '^data:image\/[a-z]+;base64,', ''
        $bytes = [Convert]::FromBase64String($clean)
        $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
        $img = [System.Drawing.Image]::FromStream($ms)
        $pic2 = New-Object Windows.Forms.PictureBox
        $pic2.Image = $img
        $pic2.SizeMode = "Zoom"
        $pic2.Size = New-Object Drawing.Size(48, 48)
        $pic2.Location = New-Object Drawing.Point(510, 10)
        $form.Controls.Add($pic2)
    } catch {
        Log "❌ Fehler beim Laden des Produktlogos: $_" "ERROR"
    }
}

# === Titel
$lblTitle = New-Object Windows.Forms.Label
$lblTitle.Text = "Microsoft Purview Automation"
$lblTitle.Font = New-Object Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Bold)
$lblTitle.AutoSize = $true
$lblTitle.Location = New-Object Drawing.Point(160, 20)
$form.Controls.Add($lblTitle)

# === Sektion: MFA Header
if ($IconStyle -eq "Online") {
    $imgMFA = Get-OnlineIcon -Url $SectionIcons["MFA"].Url -Section "MFA"
    if ($imgMFA) {
        $picMFA = New-Object Windows.Forms.PictureBox
        $picMFA.Image = $imgMFA
        $picMFA.Size = New-Object Drawing.Size(32, 32)
        $picMFA.SizeMode = "Zoom"
        $picMFA.Location = New-Object Drawing.Point(10, 70)
        $form.Controls.Add($picMFA)
    }
}
$lblMFA = New-Object Windows.Forms.Label
$lblMFA.Text = "Konfiguration MFA Anmeldung am Tenant"
$lblMFA.Font = New-Object Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
$lblMFA.AutoSize = $true
$lblMFA.Location = New-Object Drawing.Point(50, 75)
$form.Controls.Add($lblMFA)

# === UPN + Domain
$lblUPN = New-Object Windows.Forms.Label
$lblUPN.Text = "UserPrincipalName:"
$lblUPN.Location = New-Object Drawing.Point(10, 110)
$lblUPN.AutoSize = $true
$form.Controls.Add($lblUPN)

$txtUPN = New-Object Windows.Forms.TextBox
$txtUPN.Text = $UserPrincipalName
$txtUPN.Width = 400
$txtUPN.Location = New-Object Drawing.Point(10, 130)
$form.Controls.Add($txtUPN)

$lblTenant = New-Object Windows.Forms.Label
$lblTenant.Text = "Tenant-Domain:"
$lblTenant.Location = New-Object Drawing.Point(10, 160)
$lblTenant.AutoSize = $true
$form.Controls.Add($lblTenant)

$txtTenant = New-Object Windows.Forms.TextBox
$txtTenant.Text = $Tenantdomain
$txtTenant.Width = 400
$txtTenant.Location = New-Object Drawing.Point(10, 180)
$form.Controls.Add($txtTenant)

# === Sektion: Email Header
if ($IconStyle -eq "Online") {
    $imgMail = Get-OnlineIcon -Url $SectionIcons["Email"].Url -Section "Email"
    if ($imgMail) {
        $picMail = New-Object Windows.Forms.PictureBox
        $picMail.Image = $imgMail
        $picMail.Size = New-Object Drawing.Size(32, 32)
        $picMail.SizeMode = "Zoom"
        $picMail.Location = New-Object Drawing.Point(10, 220)
        $form.Controls.Add($picMail)
    }
}
$lblEmail = New-Object Windows.Forms.Label
$lblEmail.Text = "eMail Versand der Ergebnisse an:"
$lblEmail.Font = New-Object Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
$lblEmail.Location = New-Object Drawing.Point(50, 225)
$lblEmail.AutoSize = $true
$form.Controls.Add($lblEmail)

# === Primärer Mail
$lblMailPrimary = New-Object Windows.Forms.Label
$lblMailPrimary.Text = "Primärer Empfänger:"
$lblMailPrimary.Location = New-Object Drawing.Point(10, 260)
$lblMailPrimary.AutoSize = $true
$form.Controls.Add($lblMailPrimary)

$txtMailPrimary = New-Object Windows.Forms.TextBox
$txtMailPrimary.Text = $MailToPrimary
$txtMailPrimary.Width = 400
$txtMailPrimary.Location = New-Object Drawing.Point(10, 280)
$form.Controls.Add($txtMailPrimary)

# === Sekundärer Mail
$lblMailSecondary = New-Object Windows.Forms.Label
$lblMailSecondary.Text = "Optionaler 2. Empfänger (CC):"
$lblMailSecondary.Location = New-Object Drawing.Point(10, 310)
$lblMailSecondary.AutoSize = $true
$form.Controls.Add($lblMailSecondary)

$txtMailSecondary = New-Object Windows.Forms.TextBox
$txtMailSecondary.Text = $MailToSecondary
$txtMailSecondary.Width = 400
$txtMailSecondary.Location = New-Object Drawing.Point(10, 330)
$form.Controls.Add($txtMailSecondary)

# === Sektion: Log Header
if ($IconStyle -eq "Online") {
    $imgLog = Get-OnlineIcon -Url $SectionIcons["Log"].Url -Section "Log"
    if ($imgLog) {
        $picLog = New-Object Windows.Forms.PictureBox
        $picLog.Image = $imgLog
        $picLog.Size = New-Object Drawing.Size(32, 32)
        $picLog.SizeMode = "Zoom"
        $picLog.Location = New-Object Drawing.Point(10, 370)
        $form.Controls.Add($picLog)
    }
}
$lblLogHeader = New-Object Windows.Forms.Label
$lblLogHeader.Text = "Speicherort der Log / Reportdaten:"
$lblLogHeader.Font = New-Object Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
$lblLogHeader.Location = New-Object Drawing.Point(50, 375)
$lblLogHeader.AutoSize = $true
$form.Controls.Add($lblLogHeader)

# === Log-Verzeichnis
$lblLogFolder = New-Object Windows.Forms.Label
$lblLogFolder.Text = "Log- / Reportverzeichnis:"
$lblLogFolder.Location = New-Object Drawing.Point(10, 410)
$lblLogFolder.AutoSize = $true
$form.Controls.Add($lblLogFolder)

$txtLogFolder = New-Object Windows.Forms.TextBox
$txtLogFolder.Text = $LogFolder
$txtLogFolder.Width = 400
$txtLogFolder.Location = New-Object Drawing.Point(10, 430)
$form.Controls.Add($txtLogFolder)

$btnBrowse = New-Object Windows.Forms.Button
$btnBrowse.Text = "..."
$btnBrowse.Width = 30
$btnBrowse.Location = New-Object Drawing.Point(420, 430)
$form.Controls.Add($btnBrowse)

$folderDialog = New-Object Windows.Forms.FolderBrowserDialog
$btnBrowse.Add_Click({
    if ($folderDialog.ShowDialog() -eq 'OK') {
        $txtLogFolder.Text = $folderDialog.SelectedPath
    }
})

# === Footer-Info
$footer = New-Object Windows.Forms.Label
$footer.Text = @"
Bei Fragen oder Problemen wenden Sie sich an:
$MSPPartner
$MSPNameAP
$MSPMail
$MSPURL
"@
$footer.Location = New-Object Drawing.Point(10, 470)
$footer.Size = New-Object Drawing.Size(520, 60)
$footer.Font = New-Object Drawing.Font("Segoe UI", 8)
$form.Controls.Add($footer)

# === Buttons
$btnStart = New-Object Windows.Forms.Button
$btnStart.Text = "Starten"
$btnStart.Location = New-Object Drawing.Point(10, 540)
$btnStart.Width = 90
$form.Controls.Add($btnStart)

$btnCancel = New-Object Windows.Forms.Button
$btnCancel.Text = "Abbrechen"
$btnCancel.Location = New-Object Drawing.Point(110, 540)
$btnCancel.Width = 90
$form.Controls.Add($btnCancel)

# === Aktionen
$btnStart.Add_Click({
    # Pflichtfeldprüfung für primären Empfänger
    if (-not $txtMailPrimary.Text -or $txtMailPrimary.Text -eq "") {
        [System.Windows.Forms.MessageBox]::Show("Bitte geben Sie eine primäre E-Mail-Adresse an.", "Fehlende E-Mail", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        return
    }

    # Werte auslesen und in $script:Result speichern
    $script:Result = @{
        UserPrincipalName = $txtUPN.Text
        Tenantdomain      = $txtTenant.Text
        MailToPrimary     = $txtMailPrimary.Text
        MailToSecondary   = $txtMailSecondary.Text
        LogFolder         = $txtLogFolder.Text.TrimEnd('\')
    }

    # GUI-Konfiguration speichern
    $guiSave = @{
        UserPrincipalName = $txtUPN.Text
        Tenantdomain      = $txtTenant.Text
        MailToPrimary     = $txtMailPrimary.Text
        MailToSecondary   = $txtMailSecondary.Text
        LogFolder         = $txtLogFolder.Text
    }

    try {
        $guiSave | ConvertTo-Json -Depth 2 | Set-Content -Path $GuiConfigPath -Encoding UTF8
        Log "💾 GUI-Konfiguration gespeichert nach $GuiConfigPath" "INFO"
    } catch {
        Log "❌ Fehler beim Speichern der GUI-Konfiguration: $_" "ERROR"
    }

    $form.Close()
})

$btnCancel.Add_Click({
    $script:Result = $null
    $form.Close()
})

# === Formular anzeigen
$form.ShowDialog() | Out-Null

# === Auswertung
if (-not $script:Result) {
    Write-Host "Abgebrochen durch Benutzer." -ForegroundColor Red
    exit 1
}

# === Parameter übernehmen
$UserPrincipalName = $script:Result.UserPrincipalName
$Tenantdomain      = $script:Result.Tenantdomain
$MailToPrimary     = $script:Result.MailToPrimary
$MailToSecondary   = $script:Result.MailToSecondary
$LogFolder         = $script:Result.LogFolder

# === Hauptskript vorbereiten
$mainScript = ".\03-Run-Purview-Create-Documentation_GUI_Final_V3.ps1"

if (-not (Test-Path $mainScript)) {
    Write-Error "❌ Hauptskript nicht gefunden: $mainScript"
    exit 1
}

# === Argumente zusammenbauen
$arguments = @(
    "-ExecutionPolicy Bypass"
    "-STA"
    "-File `"$mainScript`""
    "-UserPrincipalName `"$UserPrincipalName`""
    "-Tenantdomain `"$Tenantdomain`""
    "-LogFolder `"$LogFolder`""
    "-MailToPrimary `"$MailToPrimary`""
    "-MailToSecondary `"$MailToSecondary`""
    "-CompanyLogoPath `"$CompanyLogoPath`""
    "-CompanyLogoUrl `"$CompanyLogoUrl`""
    "-CompanyLogoBase64 `"$CompanyLogoBase64`""
    "-LogoUrl `"$LogoUrl`""
    "-ProductLogoBase64 `"$ProductLogoBase64`""
    "-LogoGIFUrl `"$LogoGIFUrl`""
    "-MSPPartner `"$MSPPartner`""
    "-MSPNameAP `"$MSPNameAP`""
    "-MSPMail `"$MSPMail`""
    "-MSPURL `"$MSPURL`""
) -join ' '

# === Hauptskript im neuen Prozess starten (STA)
try {
    Log "🚀 Starte Hauptskript im STA-Modus: $mainScript" "INFO"
    Start-Process powershell.exe -ArgumentList $arguments -WindowStyle Normal
    Log "✅ Hauptskript gestartet" "SUCCESS"
} catch {
    Log "❌ Fehler beim Starten des Hauptskripts: $_" "ERROR"
    exit 1
}
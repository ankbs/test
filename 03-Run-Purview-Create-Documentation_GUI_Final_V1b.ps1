...
param(
    ...
    [string]$MailToPrimary = "", # erster Empfänger (z.B. -MailToSecondary "cc@firma.de")
    [string]$MailToSecondary = "", # Optionaler zweiter Empfänger (z.B. -MailToSecondary "cc@firma.de")
...
function Handle-Error {
    param(
        [string]$Message,
        [System.Object]$ErrorObject  # vorher [System.Exception]
    )

    $exceptionMessage = if ($ErrorObject -is [System.Management.Automation.ErrorRecord]) {
    ...
    Log "$Message $exceptionMessage" "ERROR"
    Write-Host "$Message $exceptionMessage" -ForegroundColor Red
    exit 1
}
...
function Start-SplashInThread {
    ...
    $script:splashThread = New-Object System.Threading.Thread({
        param (
        ...
        try {
            ...
            if ($CompanyLogoPath -and (Test-Path $CompanyLogoPath)) {
                $image = [System.Drawing.Image]::FromFile($CompanyLogoPath)
            } elseif ($CompanyLogoUrl -match '^https?://') {
                $bytes = $wc.DownloadData($CompanyLogoUrl)
                $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
                $image = [System.Drawing.Image]::FromStream($ms)
            } elseif ($CompanyLogoBase64) {
            ...
            if ($image) {
                ...
                $pic.Image = $image
                $pic.SizeMode = "Zoom"
                $pic.Location = New-Object Drawing.Point(30, 30)
                $pic.Size = New-Object Drawing.Size(100, 100)
        ...
        try {
            ...
            if ($imageProduct) {
                $pic = New-Object Windows.Forms.PictureBox
                $pic.Image = $imageProduct
                $pic.SizeMode = "Zoom"
                $pic.Location = New-Object Drawing.Point(360, 30)
                $pic.Size = New-Object Drawing.Size(100, 100)
                $form.Controls.Add($pic)
            }
        } catch {}
        ...
        if (-not $UseProgressBar) {
            try {
                $bytes = $wc.DownloadData($LogoGIFUrl)
                $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
                $gif = [System.Drawing.Image]::FromStream($ms)
                $pic = New-Object Windows.Forms.PictureBox
                $pic.Image = $gif
                $pic.SizeMode = "Zoom"
                $pic.Location = New-Object Drawing.Point(195, 30)
                $pic.Size = New-Object Drawing.Size(100, 100)
                $form.Controls.Add($pic)
            } catch {}
        } else {
    ...
    $script:splashThread.IsBackground = $true
    $script:splashThread.SetApartmentState([System.Threading.ApartmentState]::STA)

    $script:splashThread.Start(
        $CompanyLogoPath, $CompanyLogoUrl, $CompanyLogoBase64,
        $UseProgressBar, $AutoCloseAfterSeconds,
        $LogoGIFUrl, $ProductLogoBase64, $LogoUrl
    )
...
function Stop-SplashThread {
    ...
    if ($script:splashThread -and $script:splashThread.IsAlive) {
        try {
            $script:splashThread.Abort()
        } catch {}
    }
}
...
function Show-LabelSelectionForm {
    ...
    $form.Size = New-Object Drawing.Size(540, 840)
    $form.StartPosition = "CenterScreen"
    $form.Topmost = $true
...
try {
    ...
    $txtFilterCode.Location = New-Object Drawing.Point(200, 70)  # vorher: 90
    ...
    $btnApplyFilter.Location = New-Object Drawing.Point(310, 70)
    ...
    # === Sekundärer E-Mail Empfänger
    $lblMailCC = New-Object Windows.Forms.Label
    $lblMailCC.Text = "Optionaler 2. Empfänger (CC):"
    $lblMailCC.Location = New-Object Drawing.Point(10, 560)
    $lblMailCC.AutoSize = $true
    $form.Controls.Add($lblMailCC)

    $txtMailCC = New-Object Windows.Forms.TextBox
    $txtMailCC.Width = 360
    $txtMailCC.Location = New-Object Drawing.Point(10, 590)
    $txtMailCC.Text = $MailToSecondary
    $form.Controls.Add($txtMailCC)

    $txtFolder = New-Object Windows.Forms.TextBox
    $txtFolder.Text = $LogFolder
    $txtFolder.Width = 360
    $txtFolder.Location = New-Object Drawing.Point(10, 620)
...
#    $btnBrowseLog.Text = "📁"
    ...
    $btnBrowseLog.Location = New-Object Drawing.Point(380, 620)
    ...
    $btnLoadExcel.Location = New-Object Drawing.Point(420, 620)
...
"@
    ...
    $footer.Location = New-Object Drawing.Point(10, 660)  # vorher: 650
    $footer.Size = New-Object Drawing.Size(510, 100)
    $footer.AutoSize = $false
    $form.Controls.Add($footer)

    # Buttons nach unten versetzen
    $btnOK = New-Object Windows.Forms.Button
    $btnOK.Text = "OK"
    $btnOK.Width = 90
    $btnOK.Location = New-Object Drawing.Point(10, 770)  # vorher: 710
    $form.Controls.Add($btnOK)

    $btnCancel = New-Object Windows.Forms.Button
    $btnCancel.Text = "Abbrechen"
    $btnCancel.Width = 90
    $btnCancel.Location = New-Object Drawing.Point(110, 770)  # vorher: 710
...
$btnOK.Add_Click({
    ...
    $form.Tag = @{
        ...
        MailToSecondary = $txtMailCC.Text
    }
...
function Show-ErrorGui {
    ...
    $form = New-Object Windows.Forms.Form
    $form.Text = "❌ Fehler beim Ausführen"
    $form.Size = New-Object Drawing.Size(600, 240)
    $form.StartPosition = "CenterScreen"
    $form.Topmost = $true
    $form.FormBorderStyle = "FixedDialog"
    ...
    $lblTitle.Text = "Beim Ausführen des Skripts ist ein Fehler aufgetreten:"
    $lblTitle.Location = New-Object Drawing.Point(10, 10)
    $lblTitle.Size = New-Object Drawing.Size(580, 30)
    $lblTitle.Font = New-Object Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
    $form.Controls.Add($lblTitle)

    # Fehlertext
    $txtError = New-Object Windows.Forms.TextBox
    $txtError.Multiline = $true
    $txtError.ReadOnly = $true
    $txtError.ScrollBars = "Vertical"
    $txtError.Text = $Message
    $txtError.Location = New-Object Drawing.Point(10, 45)
    $txtError.Size = New-Object Drawing.Size(560, 60)
    $form.Controls.Add($txtError)

    # Logfile Pfad
    $lblLog = New-Object Windows.Forms.Label
    $lblLog.Text = "Logdatei:"
    $lblLog.Location = New-Object Drawing.Point(10, 115)
    ...
    $txtLogPath.Location = New-Object Drawing.Point(10, 135)
    $txtLogPath.Size = New-Object Drawing.Size(480, 20)
    $form.Controls.Add($txtLogPath)

    # Öffnen Button
    $btnOpenLog = New-Object Windows.Forms.Button
    $btnOpenLog.Text = "Log anzeigen"
    $btnOpenLog.Location = New-Object Drawing.Point(500, 132)
    ...
    $btnClose.Location = New-Object Drawing.Point(250, 170)
...
function Connect-MFASessions {
    try {
        ...
        Handle-Error "❌ IPPS Verbindung fehlgeschlagen:" $_
        Log "❌ Schwerer Fehler im Skript: $_" "ERROR"
        Show-ErrorGui -Message $_.Exception.Message -LogFilePath $LogFile
        exit 1
    }

    try {
        if (-not (Get-Command Get-Label -ErrorAction Stop)) {
            throw "Cmdlet 'Get-Label' ist nicht verfügbar."
        }
    } catch {
        Handle-Error "❌ Cmdlet 'Get-Label' ist nicht verfügbar:" $_
    }
}
...
if ($UseLabelGUI) {
    ...
    if ($guiResult.MailToSecondary) {
    $MailToSecondary = $guiResult.MailToSecondary
}
...
if (
    (-not $UseLabelGUI) -and
    ($LabelNames.Count -eq 0 -or $LabelNames -eq $null) -and
    (-not $Priorities) -and
...
if ($SendReport) {
    ...
    $SMTPPort = 587
    $SMTPUsername = "apikey"
    $SMTPPassword = "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"
    $MailFrom = "support-bdodigital@bdo-digital.eu"
    $MailTo = "michael.kirst-neshva@bdo-digital.eu"
    $MailToPrimary
    $MailToSecondary
...
"@
    ...
    # Empfänger bearbeiten
        $MailRecipients = @($MailTo)
            if ($MailToSecondary -and $MailToSecondary -match "^[\w\.-]+@[\w\.-]+\.\w{2,}$") {
            $MailRecipients += $MailToSecondary
            Log "✉️ Zweiter Mail-Empfänger hinzugefügt: $MailToSecondary" "DEBUG"
        }
    ...
    try {
        Send-MailMessage -From $MailFrom `
                         ...
                         -UseSsl
        Log "✅ Mail erfolgreich gesendet an $MailTo" "SUCCESS"
    }
    catch {

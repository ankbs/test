...
.DESCRIPTION
    ...
    Die Excel-Datei kann automatisch im lokalen Pfad gesucht werden (Dateiname: "Buchungskreis*.xls*"),
    per Startparameter (-ExcelFilePath oder -ExcelFileUrl) übergeben werden
    oder per Dateiauswahldialog ausgewählt werden.
    ...
    Deepl API: Bitte den eigenen API Key in $script:DeepLApiKey eintragen.
    Sensitive Label Variablen werden im Block "Vorschau- und Export-Logik" gesetzt.
    Logdaten / Exportdaten werden in Unterordnern im Ausführungsverzeichnis abgelegt.
.EXAMPLE
...
param(
    ...
    [string]$ExportFolder,
    [string]$ConfigPath = ".\GUIConfig.json",
...
# --- Config aus JSON laden (wenn vorhanden, überschreibt ggf. Param-Defaults) ---
if (Test-Path $ConfigPath) {
    try {
        $conf = Get-Content $ConfigPath -Raw | ConvertFrom-Json
        if ($conf.MSPPartner     -and -not $MSPPartner)        { $MSPPartner        = $conf.MSPPartner }
        if ($conf.MSPNameAP      -and -not $MSPNameAP)         { $MSPNameAP         = $conf.MSPNameAP }
        if ($conf.MSPMail        -and -not $MSPMail)           { $MSPMail           = $conf.MSPMail }
        if ($conf.MSPURL         -and -not $MSPURL)            { $MSPURL            = $conf.MSPURL }
        if ($conf.MSPNameEU      -and -not $MSPNameEU)         { $MSPNameEU         = $conf.MSPNameEU }
        if ($conf.UserPrincipalName -and -not $UserPrincipalName) { $UserPrincipalName = $conf.UserPrincipalName }
        if ($conf.ExportFolder   -and -not $ExportFolder)      { $ExportFolder      = $conf.ExportFolder }
    } catch {
        Write-Host "Fehler beim Laden der Konfiguration: $_"
    }
}

# --- Fallback-Defaults, wenn nach Param und Config noch leer ---
...
# $script:DeepLApiKey = "HIER_DEIN_DEEPL_API_KEY"
...
function Invoke-DeepLTranslation {
    ...
    if (-not ($DeepLSupported -contains $deeplLang)) {
        Write-DeepLLog "FALLBACK" $deeplLang $Text $null "Fallback to EN-US" "not supported by DeepL"
        $deeplLang = "EN-US"
    }
    $tries = 0
    while ($tries -lt $MaxRetries) {
        try {
            $body = "text=$(Invoke-UrlEncode $Text)&target_lang=$deeplLang&source_lang=DE"
            Write-DeepLLog "REQUEST" $deeplLang $Text $body $null $null
            ...
            $stream.Close()
            $response.Close()
            Write-DeepLLog "RESPONSE" $deeplLang $Text $body $result $null
            ...
            if ($_.Exception.Response) {
                ...
                Write-DeepLLog "ERROR" $deeplLang $Text $body $errorBody $errorMsg
            } else {
                Write-DeepLLog "ERROR" $deeplLang $Text $body $null $errorMsg
            }
...
Ensure-RequiredModule -ModuleName "ImportExcel"
Import-Module ImportExcel -Force

function Show-ExcelFileDialog {
...
elseif ($ExcelFileUrl -and $ExcelFileUrl -match "^https?://") {
    ...
    if ($excelFile) {
        ...
        $selectedPath = Show-ExcelFileDialog
...
try {
    ...
    $excelData = $excelData | Where-Object { $_.Aktiv -eq 1 }
...
# ======= XAML-Definition inkl. neuer Blocking/Unblocking Sektion =======
$xaml = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        Title="Label Provisioning" Height="1000" Width="1100" WindowStartupLocation="CenterScreen">
    <Grid>
        <Grid.RowDefinitions>
            ...
            <RowDefinition Height="auto"/>
            <!-- <RowDefinition Height="auto"/> -->
            <RowDefinition Height="80"/> <!-- Footer-Höhe anpassen -->
        </Grid.RowDefinitions>
        ...
        <Grid Grid.Row="0" Background="#EEF1F6">
            ...
            <Image Name="imgCompanyLogo" Height="56" Width="56" Margin="12"/>
            <TextBlock Grid.Column="1" Text="Security &amp; Compliance - Microsoft Purview Automation" FontSize="32" FontWeight="Bold" 
                       Foreground="#003366" HorizontalAlignment="Center" VerticalAlignment="Center"/>
            <Image Grid.Column="2" Name="imgProductLogo" Height="56" Width="56" Margin="12" HorizontalAlignment="Right"/>
        </Grid>
        ...
        <StackPanel Grid.Row="1" Margin="10,10,10,10">
            ...
            <!-- Export/Translate/Abbrechen/Abbruch -->
            <Grid Margin="0,0,0,0">
                <Grid.ColumnDefinitions>
                ...
                <Button Name="btnAbortTranslate" Grid.Column="2" Width="170" Height="32" Content="Übersetzung abbrechen" Margin="0,0,10,0"/>
                <Button Name="btnCancel" Grid.Column="3" Width="140" Height="32" Content="Abbrechen" HorizontalAlignment="Right" />
            </Grid>
            <!-- Block/Unblock + Hyperlink + Separator -->
            <Grid Margin="0,18,0,0">
                ...
                <StackPanel Grid.Row="0" Grid.Column="0" Margin="0,0,10,0">
                    <TextBlock Text="Automatische Verarbeitung durch CoPilot unterbinden"
                               FontWeight="Bold" FontSize="14" Margin="0,0,0,8"/>
                    <StackPanel Orientation="Horizontal">
                        <Button Name="btnBlockAll" Width="140" Height="32" Margin="0,0,12,0" Content="Alle Labels"/>
                        <Button Name="btnBlockEncrypted" Width="180" Height="32" Content="Nur verschlüsselte Labels"/>
                    </StackPanel>
                </StackPanel>

                <!-- Einschränkung Rückgängig machen -->
                <StackPanel Grid.Row="0" Grid.Column="1" Margin="10,0,0,0">
                    <TextBlock Text="Einschränkung Rückgängig machen"
                               FontWeight="Bold" FontSize="14" Margin="0,0,0,8"/>
                    <StackPanel Orientation="Horizontal">
                        <Button Name="btnUnblockAll" Width="140" Height="32" Margin="0,0,12,0" Content="Alle Labels"/>
                        <Button Name="btnUnblockEncrypted" Width="180" Height="32" Content="Nur verschlüsselte Labels"/>
                    </StackPanel>
                </StackPanel>

                <!-- Hinweis/Hyperlink (über beide Spalten) -->
                <TextBlock Grid.Row="1" Grid.Column="0" Grid.ColumnSpan="2" Margin="0,12,0,0">
                    <Run Text="Achtung: Bitte lesen Sie folgende Hinweise bezüglich der Einschrämkungen:" />
                    <LineBreak/>
                    <Hyperlink Name="hlWarnung"
                    NavigateUri="https://learn.microsoft.com/de-de/purview/sensitivity-labels-office-apps#prevent-some-connected-experiences-that-analyze-content">
                    https://learn.microsoft.com/de-de/purview/sensitivity-labels-office-apps#prevent-some-connected-experiences-that-analyze-content
                    </Hyperlink>
                </TextBlock>

                <!-- Horizontale Linie / Separator -->
                <Separator Grid.Row="2" Grid.Column="0" Grid.ColumnSpan="2" Margin="0,12,0,6"/>
            </Grid>
        </StackPanel>
        ...
        <Grid Grid.Row="4" Background="#EEF1F6" Margin="0,0,0,0">
            <Grid.RowDefinitions>
                <RowDefinition Height="*"/>
                <RowDefinition Height="*"/>
                <RowDefinition Height="*"/>
            ...
            <TextBlock Grid.Row="1"
                Text="Bei Fragen zu diesem Tool wenden Sie sich bitte an Ihren Microsoft Solution Partner."
                VerticalAlignment="Center" HorizontalAlignment="Center" FontSize="15" Foreground="#444"/>
            <!-- Zeile 4: MSP Infos -->
            <StackPanel Grid.Row="3" Orientation="Horizontal" HorizontalAlignment="Center" VerticalAlignment="Center">
                <TextBlock Text="🏢" FontSize="18" Margin="10,0,2,0"/>
                <TextBlock Name="txtMSP" Text="" FontSize="14" Margin="2,0,20,0" VerticalAlignment="Center"/>
                <TextBlock Text="📧" FontSize="18" Margin="10,0,2,0"/>
                <TextBlock Name="txtMail" Text="" FontSize="14" Margin="2,0,20,0" VerticalAlignment="Center" Foreground="#0078d7" TextDecorations="Underline" Cursor="Hand"/>
                <TextBlock Text="🌐" FontSize="18" Margin="10,0,2,0"/>
                <TextBlock Name="txtURL" Text="" FontSize="14" Margin="2,0,0,0" VerticalAlignment="Center" Foreground="#0078d7" TextDecorations="Underline" Cursor="Hand"/>
            </StackPanel>
...
$btnAbortTranslate = $window.FindName('btnAbortTranslate')
$btnCancel = $window.FindName('btnCancel')
$btnBlockAll = $window.FindName('btnBlockAll')
$btnBlockEncrypted = $window.FindName('btnBlockEncrypted')
$btnUnblockAll = $window.FindName('btnUnblockAll')
$btnUnblockEncrypted = $window.FindName('btnUnblockEncrypted')
...
# Annahme: $window wurde über XAML geladen
$hlWarnung = $window.FindName("hlWarnung")
if ($hlWarnung) {
    $hlWarnung.Add_Click({
        Start-Process $hlWarnung.NavigateUri.AbsoluteUri
    })
}

# Übersetzungsabbruch-Button
$btnAbortTranslate.Add_Click({
    $script:abortTranslation = $true
})

# Abbrechen-Button (setzt auch abortTranslation)
$btnCancel.Add_Click({
    $script:abortTranslation = $true
    $script:closing = $true
    $window.Close()
...
Set-ControlImage $imgCompanyLogo $CompanyLogoBase64
Set-ControlImage $imgProductLogo $ProductLogoBase64
...
function New-LabelObjects {
    param(
        $bk, $company, $maildomain, $AdvancedSettings = $null, $LabelSelector = "all"
    )
    ...
    foreach ($label in $labelTemplates) {
        if ($LabelSelector -eq "encrypted" -and $label -notlike "*verschlüsselt*" -and $label -ne "Streng Vertraulich") { continue }
        $obj = [PSCustomObject]@{
            Name        = "$label - $bk4"
            ...
            ContentType = "File, Email,SchematizedData"
        }
        ...
        if ($AdvancedSettings -and $AdvancedSettings.ContainsKey('BlockContentAnalysisServices')) {
            $obj | Add-Member NoteProperty BlockContentAnalysisServices $AdvancedSettings.BlockContentAnalysisServices
        }
        $out += $obj
    }
    return $out
...
$script:currentBlockingMode = $null
$script:currentBlockingSelection = $null

function Update-LabelPreviewAndExport {
    param($BlockMode, $LabelSelector)
    if ($script:closing) { return }
    $script:currentBlockingMode = $BlockMode
    $script:currentBlockingSelection = $LabelSelector
    ...
    if ($selBK) {
        ...
        if ($entry) {
            ...
            $blockVal = $null
            $prefix = "provision_labels"
            if ($BlockMode -eq "block") {
                $blockVal = $true
                $prefix = "${prefix}_with-blocking"
            }
            elseif ($BlockMode -eq "unblock") {
                $blockVal = $false
                $prefix = "${prefix}_no-blocking"
            }
            $labels = New-LabelObjects $selBK $company $maildomain @{BlockContentAnalysisServices=$blockVal} $LabelSelector
            $tbPreview.Text = Get-LabelPreviewText $labels
            $window.Tag = $labels
            $bk4 = $selBK.ToString().PadLeft(4,'0')
            $filename = Join-Path $ExportFolder "${prefix}_${bk4}.json"
            $jsonString = $labels | ConvertTo-Json -Depth 4
            [System.IO.File]::WriteAllText($filename, $jsonString, $utf8NoBom)
            $global:filenameExportJSON = $filename
            [System.Windows.MessageBox]::Show("JSON exportiert:\n$filename")
        }
    } else {
        $tbPreview.Text = ""
        $window.Tag = $null
...
$btnBlockAll.Add_Click({ Update-LabelPreviewAndExport "block" "all" })
$btnBlockEncrypted.Add_Click({ Update-LabelPreviewAndExport "block" "encrypted" })
$btnUnblockAll.Add_Click({ Update-LabelPreviewAndExport "unblock" "all" })
$btnUnblockEncrypted.Add_Click({ Update-LabelPreviewAndExport "unblock" "encrypted" })

$cbBkreis.Add_SelectionChanged({
    if ($script:closing) { return }
    if ($script:currentBlockingMode -and $script:currentBlockingSelection) {
        Update-LabelPreviewAndExport $script:currentBlockingMode $script:currentBlockingSelection
    } else {
        Update-LabelPreviewAndExport $null "all"
    }
})
...
$btnExportCSV.Add_Click({
    ...
    if ($labels -and $selBK) {
        ...
        [System.Windows.MessageBox]::Show("CSV exportiert:\n$filename")
    } else {
...
# Im btnTranslate-Handler:
$btnTranslate.Add_Click({
    if ($script:closing) { return }
    $script:abortTranslation = $false
    $btnTranslate.IsEnabled = $false
    $btnAbortTranslate.IsEnabled = $true
    ...
    try {
        ...
        foreach ($label in $labels) {
            if ($script:abortTranslation) {
                $tbStatus.Text = "Übersetzung abgebrochen!"
                break
            }
            $item = [ordered]@{ }
            ...
            foreach ($lang in $targetLangs.Keys) {
                ...
                $item["DisplayName_$lang"] = ConvertTo-Utf8String (Invoke-DeepLTranslation $label.DisplayName $deeplLang)
                ...
                $item["Tooltip_$lang"] = ConvertTo-Utf8String (Invoke-DeepLTranslation $label.Tooltip $deeplLang)
                $done++; $pbTranslate.Value = [int](($done/$total)*100)
            ...
            if ($script:abortTranslation) { break }
        }
        if ($script:abortTranslation) {
            $tbStatus.Text = "Übersetzung abgebrochen!"
        } else {
            ...
            [System.IO.File]::WriteAllText($filename, $jsonString, $utf8NoBom)
            [System.Windows.MessageBox]::Show("Übersetzung abgeschlossen:\n$filename")
        ...
        $btnAbortTranslate.IsEnabled = $false
...
$btnCreateLabels.Add_Click({
    if ($script:closing) { return }
    if (-not $global:filenameExportJSON -or $global:filenameExportJSON -eq "") {
        [System.Windows.MessageBox]::Show("Bitte zuerst einen JSON-Export durchführen!", "Fehler", "OK", "Error")
        return
    }
    $scriptName = "02-Run-PurviewLabelProvisioning_Create_Missing_Config_Only_Language_Final_V1.ps1"
    $arguments = @("-ExecutionPolicy Bypass", "-STA", "-File `"$scriptName`"", "-UserPrincipalName `"$UserPrincipalName`" -Tenantdomain `"$Tenantdomain`" -ExportFolder `"$global:filenameExportJSON`"") -join " "
    Write-Host "Starte: $scriptName $arguments" -ForegroundColor Cyan
...
$btnMoreOptions.Add_Click({
    if ($script:closing) { return }
    $scriptName = "00_a_Start-PurviewGUI_V1.ps1"

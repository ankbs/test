...
# $LogoGIFUrl    = "https://i.gifer.com/ZKZg.gif" # "", # "https://i.gifer.com/ZKZg.gif",  # animiertes Zahnrad
        ...
        Write-Host "DEBUG: LogoGIFUrl (vor Prüfung): $LogoGIFUrl" -ForegroundColor Cyan
...
function Show-LabelSelectionForm {
    ...
    $form.Size = New-Object Drawing.Size(540, 880)
    $form.StartPosition = "CenterScreen"
    $form.Topmost = $true
...
try {
    ...
    $lblFilter.Text = "Filter nach Buchungskrei (z.B. 0345):"
    ...
    # === Optionen (nebeneinander auf gleicher Höhe)
    $chkWord = New-Object Windows.Forms.CheckBox
    $chkWord.Text = "Word-Bericht"
    $chkWord.Checked = $true
    $chkWord.AutoSize = $true
    $chkWord.Location = New-Object Drawing.Point(10, 530)
    ...
    $chkPDF.Text = "PDF-Bericht"
    $chkPDF.Checked = $false
    $chkPDF.AutoSize = $true
    $chkPDF.Location = New-Object Drawing.Point(150, 530)
    ...
    $chkMail.Text = "Per Mail senden"
    $chkMail.Checked = $false
    $chkMail.AutoSize = $true
    $chkMail.Location = New-Object Drawing.Point(270, 530)
    $form.Controls.Add($chkMail)
    $toolTip.SetToolTip($chkMail, "Versende einen eMail Report mit allen Dateien (SMTP Sendgrid erforderlich)")


    $chkWordC = New-Object Windows.Forms.CheckBox
    $chkWordC.Text = "Report per Company"
    $chkWordC.Checked = $false
    $chkWordC.AutoSize = $true
    $chkWordC.Location = New-Object Drawing.Point(390, 530)
    $form.Controls.Add($chkWordC)
    $toolTip.SetToolTip($chkWordC, "Erstelle einen Report pro Company")



    # === Primärer E-Mail Empfänger
    $lblMailTo = New-Object Windows.Forms.Label
    $lblMailTo.Text = "Primärer Empfänger (To):"
    $lblMailTo.Location = New-Object Drawing.Point(10, 560)
    $lblMailTo.AutoSize = $true
    $form.Controls.Add($lblMailTo)

    $txtMailTo = New-Object Windows.Forms.TextBox
    $txtMailTo.Width = 360
    $txtMailTo.Location = New-Object Drawing.Point(10, 590)
    $txtMailTo.Text = $MailToPrimary
    $form.Controls.Add($txtMailTo)

# Abstände anpassen – verschiebe CC und Folder weiter nach unten!
#    $lblMailCC.Location = New-Object Drawing.Point(10, 580)
#    $txtMailCC.Location = New-Object Drawing.Point(10, 600)
#    $txtFolder.Location = New-Object Drawing.Point(10, 630)
#    $btnBrowseLog.Location = New-Object Drawing.Point(380, 630)
#    $btnLoadExcel.Location = New-Object Drawing.Point(420, 630)
    ...
    $lblMailCC.Location = New-Object Drawing.Point(10, 620)
    $lblMailCC.AutoSize = $true
    $form.Controls.Add($lblMailCC)

    $txtMailCC = New-Object Windows.Forms.TextBox
    $txtMailCC.Width = 360
    $txtMailCC.Location = New-Object Drawing.Point(10, 650)
    ...
    $txtFolder.Width = 360
    $txtFolder.Location = New-Object Drawing.Point(10, 680)
...
#    $btnBrowseLog.Text = "📁"
    ...
    $btnBrowseLog.Location = New-Object Drawing.Point(380, 680)
    ...
    $btnLoadExcel.Location = New-Object Drawing.Point(420, 680)
...
# === Footer-Text (nur statische Elemente)
...
$lblFooter.Location = New-Object Drawing.Point(10, 710)
$lblFooter.Size = New-Object Drawing.Size(500, 40)
$lblFooter.AutoSize = $false
$form.Controls.Add($lblFooter)

# === LinkLabel: E-Mail
$linkMail = New-Object Windows.Forms.LinkLabel
$linkMail.Text = $MSPMail
$linkMail.Location = New-Object Drawing.Point(10, 760)
$linkMail.AutoSize = $true
$linkMail.LinkColor = "Blue"
$linkMail.Font = New-Object Drawing.Font("Segoe UI", 8, [System.Drawing.FontStyle]::Underline)
$linkMail.Add_LinkClicked({
    Start-Process "mailto:$MSPMail"
})
$form.Controls.Add($linkMail)

# === LinkLabel: Webseite
$linkWeb = New-Object Windows.Forms.LinkLabel
$linkWeb.Text = $MSPURL
$linkWeb.Location = New-Object Drawing.Point(10, 780)
$linkWeb.AutoSize = $true
$linkWeb.LinkColor = "Blue"
$linkWeb.Font = New-Object Drawing.Font("Segoe UI", 8, [System.Drawing.FontStyle]::Underline)
$linkWeb.Add_LinkClicked({
    Start-Process "$MSPURL"
})
$form.Controls.Add($linkWeb)
# === LinkLabel: Dokumentation
    ...
    $btnOK.Text = "OK"
    $btnOK.Width = 90
    $btnOK.Location = New-Object Drawing.Point(10, 810)  # vorher: 710
    $form.Controls.Add($btnOK)

    $btnCancel = New-Object Windows.Forms.Button
    $btnCancel.Text = "Abbrechen"
    $btnCancel.Width = 90
    $btnCancel.Location = New-Object Drawing.Point(110, 810)  # vorher: 710
...
$btnOK.Add_Click({
    ...
    $form.Tag = @{
        ...
        ExportWordC = $chkWordC.Checked
        LogFolder   = $txtFolder.Text
        MailToPrimary   = $txtMailTo.Text
...
# === Ende der Konfigurationen und Funktionen ===
...
if ($UseLabelGUI) {
    ...
    $ExportWordC = $guiResult.ExportWordC
...
function Export-CompanyReports {
    ...
    # Gruppierung der Labels nach 4-stelligem Buchungscode (am Anfang oder Ende)
    $groupedLabels = $Labels | Group-Object {
        if ($_.DisplayName -match '^(\d{4})') {
            $matches[1]
        }
        elseif ($_.DisplayName -match '(\d{4})$') {
            $matches[1]
        }
        else {
            "0000"
    ...
    foreach ($group in $groupedLabels) {
        ...
        # Firmenname aus Tooltip (einheitlich pro Block)
        $companyName = ($LabelsBlock | Where-Object { $_.Tooltip -match ' - (.+)$' } | Select-Object -First 1).Tooltip -replace '.* - ', ''
        ...
        # Hilfsfunktion zur Dateinamen-Bereinigung
        function Convert-CompanyNameToFileName {
            ...
            $Name = $Name -replace '[ &]', ''     # Leerzeichen UND & durch _
            # Optional: Entferne weitere nicht-erlaubte Dateisystemzeichen:
            $Name = $Name -replace '[^a-zA-Z0-9_\-]', '_'
            return $Name
        }

        # CompanyName aufbereiten:
        ...
        # $WordPath = [System.IO.Path]::Combine($LogFolder, "PurviewExport_${companyNameFile}-$Buchungscode-$datum.docx")
        # $PdfPath  = [System.IO.Path]::Combine($LogFolder, "PurviewExport_${companyNameFile}-$Buchungscode-$datum.pdf")

        # $WordPath = [System.IO.Path]::Combine($LogFolder, "PurviewExport_$companyName-$Buchungscode-$datum.docx")
        # $PdfPath  = [System.IO.Path]::Combine($LogFolder, "PurviewExport_$companyName-$Buchungscode-$datum.pdf")
...
#        $selection.TypeParagraph()
#        $selection.TypeText("Erstellt am: $(Get-Date -Format 'dd.MM.yyyy')")
#        $selection.InsertNewPage()

#        # Inhalt pro Label
#        foreach ($label in $LabelsBlock) {
#            $selection.Style = "Überschrift 1"
#            $selection.TypeText($label.DisplayName)
#            $selection.TypeParagraph()
#
#            $selection.Style = "Standard"
#            $selection.TypeText("Tooltip: $($label.Tooltip)")
#            $selection.TypeParagraph()
#            $selection.TypeText("Priority: $($label.Priority)")
#            $selection.TypeParagraph()
#            $selection.TypeText("Description: $($label.Description)")
#            $selection.TypeParagraph()
#            $selection.InsertNewPage()
#        }
...
#            $selection.TypeParagraph()
    ...
    $selection.InsertNewPage()
        ...
        Write-Host "✅ Bericht für $companyName ($Buchungscode) erstellt: $wordPath & $pdfPath"
    }
}

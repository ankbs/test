# === Parameter definieren ===
param (
    [string]$UserPrincipalName = "user@domain.tld",
    [string]$Tenantdomain      = "domain.tld",
    [string]$CompanyLogoPath = "",
    [string]$CompanyLogoUrl = "",
    [string]$ProductLogoPath = "",
    [string]$ProductLogoUrl = "",
    [string]$LogoUrl = "",
    [string]$LogoGIFUrl = "",
    [string]$MailToPrimary = "michael.kirst-neshva@bdo-digital.eu",
    [string]$MailToSecondary = "",
    [string]$LogFolder = "C:\Temp\script\",
    [string]$MSPPartner = "Cloud Security && Compliance Services",
    [string]$MSPNameAP  = "Michael Kirst-Neshva",
    [string]$MSPMail    = "support@domainen.io",
    [string]$MSPURL     = "https://www.domainen.io",
    [string]$MSPNameEU  = "Michael Kirst-Neshva",
    [string]$CompanyLogoBase64 = "data:image/png;base64,", # Companylogo als eingebettetes Base64
    [string]$ProductLogoBase64 = "data:image/png;base64," # Produktlogo als eingebettetes Base64

)

if (!(Test-Path $LogFolder)) { New-Item -ItemType Directory -Path $LogFolder -Force | Out-Null }


# === Zeitstempel / Logdateien ===
$DatumJetzt = Get-Date -Format 'yyyyMMdd_HHmmss'
$DatumAnzeige = Get-Date -Format 'dd.MM.yyyy'
$cleanUser = $UserPrincipalName -replace '[^a-zA-Z0-9]', '_'
# $LogFolder = $LogFolder.TrimEnd('\')
#  $LogFile = [System.IO.Path]::Combine($LogFolder, "Label_Status_LOG_$DatumJetzt.log")
$CreatedLabelsCsv = [System.IO.Path]::Combine($LogFolder, "Erstellte_Labels_$DatumJetzt.csv")
$StatusReportCsv = [System.IO.Path]::Combine($LogFolder, "Label_Status_$DatumJetzt.csv")


# === Konfigurationsdatei für GUI-Settings ===
$GuiConfigPath = Join-Path $PSScriptRoot "GUIConfig.json"

# === Logging + Error-Handling ===
function Log {
    param([string]$Message, [string]$Level = "INFO")
    $timestamp = Get-Date -Format 'yyyy-MM-dd HH:mm:ss'
    $prefix = switch ($Level) {
        "INFO"     { "ℹ️" }
        "SUCCESS"  { "✅" }
        "ERROR"    { "❌" }
        "DEBUG"    { "🐛" }
        default    { "🔹" }
    }
    $logEntry = "$timestamp $prefix $Message"
    Add-Content -Path (Join-Path $LogFolder "StartGUI_$DatumJetzt.log") -Value $logEntry -Encoding utf8
    Write-Host $logEntry -Encoding utf8
}

Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing

# === Vorbelegung aus GUIConfig.json (falls vorhanden)
if (Test-Path $GuiConfigPath) {
    try {
        $GuiConfig = Get-Content -Raw -Path $GuiConfigPath | ConvertFrom-Json
        if ($GuiConfig.UserPrincipalName) { $UserPrincipalName = $GuiConfig.UserPrincipalName }
        if ($GuiConfig.Tenantdomain)      { $Tenantdomain      = $GuiConfig.Tenantdomain }
        if ($GuiConfig.MailToPrimary)     { $MailToPrimary     = $GuiConfig.MailToPrimary }
        if ($GuiConfig.MailToSecondary)   { $MailToSecondary   = $GuiConfig.MailToSecondary }
        if ($GuiConfig.LogFolder)         { $LogFolder         = $GuiConfig.LogFolder }
        Log "📥 GUI-Konfiguration aus $GuiConfigPath geladen." "INFO"
    } catch {
        Log "⚠️ Fehler beim Einlesen der GUI-Konfiguration: $_" "ERROR"
    }
}

$form = New-Object Windows.Forms.Form
$form.Text = "Microsoft Purview – Startkonfiguration"
$form.Size = New-Object Drawing.Size(580, 780)
$form.StartPosition = "CenterScreen"
$form.Topmost = $true
$form.Font = New-Object Drawing.Font("Segoe UI Emoji", 9)

# Tooltip-Unterstützung
$toolTip = New-Object System.Windows.Forms.ToolTip
$toolTip.IsBalloon = $true
$toolTip.ShowAlways = $true


# === Company-Logo ===
if ($CompanyLogoBase64 -and $CompanyLogoBase64.Length -gt 100) {
    try {
        $clean = $CompanyLogoBase64 -replace '^data:image\/[a-z]+;base64,', ''
        $bytes = [Convert]::FromBase64String($clean)
        $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
        $img = [System.Drawing.Image]::FromStream($ms)
        $pic = New-Object Windows.Forms.PictureBox
        $pic.Image = $img
        $pic.SizeMode = "Zoom"
        $pic.Size = New-Object Drawing.Size(48, 48)
        $pic.Location = New-Object Drawing.Point(10, 10)
        $form.Controls.Add($pic)
    } catch {
        Log "❌ Fehler beim Laden des Companylogos: $_" "ERROR"
    }
}

# === Produktlogo ===
if ($ProductLogoBase64 -and $ProductLogoBase64.Length -gt 100) {
    try {
        $clean = $ProductLogoBase64 -replace '^data:image\/[a-z]+;base64,', ''
        $bytes = [Convert]::FromBase64String($clean)
        $ms = New-Object IO.MemoryStream (,[byte[]]$bytes)
        $img = [System.Drawing.Image]::FromStream($ms)
        $pic2 = New-Object Windows.Forms.PictureBox
        $pic2.Image = $img
        $pic2.SizeMode = "Zoom"
        $pic2.Size = New-Object Drawing.Size(48, 48)
        $pic2.Location = New-Object Drawing.Point(510, 10)
        $form.Controls.Add($pic2)
    } catch {
        Log "❌ Fehler beim Laden des Produktlogos: $_" "ERROR"
    }
}


# Titel
$lblTitle = New-Object Windows.Forms.Label
$lblTitle.Text = "Microsoft Purview Automation"
$lblTitle.Font = New-Object Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Bold)
$lblTitle.AutoSize = $true
$lblTitle.Location = New-Object Drawing.Point(160, 20)
$form.Controls.Add($lblTitle)

# === Abschnitt: MFA Anmeldung ========================================
$lblHeaderMFA = New-Object Windows.Forms.Label
$lblHeaderMFA.Text = "🔐 Anmeldung für Purview Compliance Portal per MFA"
$lblHeaderMFA.Font = New-Object Drawing.Font("Segoe UI", 10, [Drawing.FontStyle]::Bold)
$lblHeaderMFA.AutoSize = $true
$lblHeaderMFA.Location = New-Object Drawing.Point(10, 100)
$form.Controls.Add($lblHeaderMFA)

$lblUPN = New-Object Windows.Forms.Label
$lblUPN.Text = "Benutzer-Email (UPN):"
$lblUPN.Location = New-Object Drawing.Point(10, 140)
$lblUPN.Width = 150
$lblUPN.Height = 22
$form.Controls.Add($lblUPN)

$txtUPN = New-Object Windows.Forms.TextBox
$txtUPN.Width = 300
$txtUPN.Location = New-Object Drawing.Point(210, 138)
$txtUPN.Text = $UserPrincipalName
$form.Controls.Add($txtUPN)
$toolTip.SetToolTip($txtUPN, "Anmeldung am M365 Tenant, bei Bedarf UPN überschreiben")

$lblTenant = New-Object Windows.Forms.Label
$lblTenant.Text = "Tenant Domain:"
$lblTenant.Location = New-Object Drawing.Point(10, 170)
$form.Controls.Add($lblTenant)

$txtTenant = New-Object Windows.Forms.TextBox
$txtTenant.Width = 300
$txtTenant.Location = New-Object Drawing.Point(210, 168)
$txtTenant.Text = $Tenantdomain
$form.Controls.Add($txtTenant)
$toolTip.SetToolTip($txtTenant, "Name der M365 Domain")

# Trennlinie
$line1 = New-Object Windows.Forms.Panel
$line1.Height = 2
$line1.Width = 500
$line1.BackColor = [System.Drawing.Color]::LightGray
$line1.Location = New-Object Drawing.Point(10, 200)
$form.Controls.Add($line1)

# === Abschnitt: Mail-Empfänger ========================================
$lblHeaderMail = New-Object Windows.Forms.Label
$lblHeaderMail.Text = "📧 eMail Empfänger für Berichte"
$lblHeaderMail.Font = New-Object Drawing.Font("Segoe UI", 10, [Drawing.FontStyle]::Bold)
$lblHeaderMail.AutoSize = $true
$lblHeaderMail.Location = New-Object Drawing.Point(10, 220)
$form.Controls.Add($lblHeaderMail)

$lblMailPrimary = New-Object Windows.Forms.Label
$lblMailPrimary.Text = "Primärer Empfänger:"
$lblMailPrimary.Location = New-Object Drawing.Point(10, 250)
$lblMailPrimary.Width = 150
$lblMailPrimary.Height = 22
$form.Controls.Add($lblMailPrimary)

$txtMailPrimary = New-Object Windows.Forms.TextBox
$txtMailPrimary.Width = 300
$txtMailPrimary.Location = New-Object Drawing.Point(210, 248)
$txtMailPrimary.Text = $MailToPrimary
$form.Controls.Add($txtMailPrimary)
$toolTip.SetToolTip($txtMailPrimary, "An wen sollen die Berichte zur Kontrolle gesendet werden?")

$lblMailSecondary = New-Object Windows.Forms.Label
$lblMailSecondary.Text = "Sekundärer Empfänger (optional):"
$lblMailSecondary.Location = New-Object Drawing.Point(10, 280)
$lblMailSecondary.Width = 150
$lblMailSecondary.Height = 40
$form.Controls.Add($lblMailSecondary)

$txtMailSecondary = New-Object Windows.Forms.TextBox
$txtMailSecondary.Width = 300
$txtMailSecondary.Location = New-Object Drawing.Point(210, 278)
$form.Controls.Add($txtMailSecondary)
$toolTip.SetToolTip($txtMailSecondary, "Optionaler zusätzlicher Empfänger in CC")

# Trennlinie
$line2 = New-Object Windows.Forms.Panel
$line2.Height = 2
$line2.Width = 500
$line2.BackColor = [System.Drawing.Color]::LightGray
$line2.Location = New-Object Drawing.Point(10, 330)
$form.Controls.Add($line2)

# === Abschnitt: Speicherorte ==========================================
$lblHeaderStorage = New-Object Windows.Forms.Label
$lblHeaderStorage.Text = "💾 Speicherorte"
$lblHeaderStorage.Font = New-Object Drawing.Font("Segoe UI", 10, [Drawing.FontStyle]::Bold)
$lblHeaderStorage.AutoSize = $true
$lblHeaderStorage.Location = New-Object Drawing.Point(10, 350)
$form.Controls.Add($lblHeaderStorage)

$lblLogFolder = New-Object Windows.Forms.Label
$lblLogFolder.Text = "Log- und Bericht-Ordner:"
$lblLogFolder.Location = New-Object Drawing.Point(10, 380)
$lblLogFolder.Width = 150
$lblLogFolder.Height = 22
$form.Controls.Add($lblLogFolder)

$txtLogFolder = New-Object Windows.Forms.TextBox
$txtLogFolder.Width = 250
$txtLogFolder.Location = New-Object Drawing.Point(210, 378)
$txtLogFolder.Text = $LogFolder
$form.Controls.Add($txtLogFolder)

$btnBrowse = New-Object Windows.Forms.Button
$btnBrowse.Text = "..."
$btnBrowse.Width = 30
$btnBrowse.Location = New-Object Drawing.Point(480, 378)
$form.Controls.Add($btnBrowse)

$folderDialog = New-Object System.Windows.Forms.FolderBrowserDialog
$btnBrowse.Add_Click({
    if ($folderDialog.ShowDialog() -eq 'OK') {
        $txtLogFolder.Text = $folderDialog.SelectedPath
    }
})

# Trennlinie Action
$lineAction = New-Object Windows.Forms.Panel
$lineAction.Height = 2
$lineAction.Width = 500
$lineAction.BackColor = [System.Drawing.Color]::LightGray
$lineAction.Location = New-Object Drawing.Point(10, 430)
$form.Controls.Add($lineAction)

# === RadioButtons für Aktionen ======================================
$radReport = New-Object Windows.Forms.RadioButton
$radReport.Text = "Label-Report"
$radReport.Checked = $true
$radReport.Location = New-Object Drawing.Point(10, 460)
$radReport.AutoSize = $true
$form.Controls.Add($radReport)

$radAddLanguage = New-Object Windows.Forms.RadioButton
$radAddLanguage.Text = "Label-Sprachen"
$radAddLanguage.Location = New-Object Drawing.Point(110, 460)
$radAddLanguage.AutoSize = $true
$form.Controls.Add($radAddLanguage)

$radSortieren = New-Object Windows.Forms.RadioButton
$radSortieren.Text = "Label-Sortieren"
$radSortieren.Location = New-Object Drawing.Point(230, 460)
$radSortieren.AutoSize = $true
$form.Controls.Add($radSortieren)

# Trennlinie Footer
$lineFooter = New-Object Windows.Forms.Panel
$lineFooter.Height = 2
$lineFooter.Width = 500
$lineFooter.BackColor = [System.Drawing.Color]::LightGray
$lineFooter.Location = New-Object Drawing.Point(10, 590)
$form.Controls.Add($lineFooter)

# === Footer ===========================================================
$lblFooter = New-Object Windows.Forms.Label
$lblFooter.Text = "Bei Fragen oder Problemen, wenden Sie sich bitte an:`n$MSPPartner`n$MSPNameAP"
$lblFooter.Font = New-Object Drawing.Font("Segoe UI", 8)
$lblFooter.Location = New-Object Drawing.Point(10, 600)
$lblFooter.Size = New-Object Drawing.Size(520, 40)
$form.Controls.Add($lblFooter)

# === Buttons
$btnStart = New-Object Windows.Forms.Button
$btnStart.Text = "Report Starten"
$btnStart.Location = New-Object Drawing.Point(10, 710)
$btnStart.Width = 90
$form.Controls.Add($btnStart)

$btnCancel = New-Object Windows.Forms.Button
$btnCancel.Text = "Abbrechen"
$btnCancel.Location = New-Object Drawing.Point(110, 710)
$btnCancel.Width = 90
$form.Controls.Add($btnCancel)

# === Aktionen
$btnStart.Add_Click({
    if (-not $txtMailPrimary.Text -or $txtMailPrimary.Text -eq "") {
        [System.Windows.Forms.MessageBox]::Show("Bitte geben Sie eine primäre E-Mail-Adresse an.", "Fehlende E-Mail", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        return
    }

    if ($radReport.Checked) {
        $aktion = "Report"
    } elseif ($radAddLanguage.Checked) {
        $aktion = "AddLanguage"
    } elseif ($radSortieren.Checked) {
        $aktion = "Sortieren"
    } else {
        $aktion = "Unbekannt"
    }

    $script:Result = @{
        UserPrincipalName = $txtUPN.Text
        Tenantdomain      = $txtTenant.Text
        MailToPrimary     = $txtMailPrimary.Text
        MailToSecondary   = $txtMailSecondary.Text
        LogFolder         = $txtLogFolder.Text.TrimEnd('\')
        Aktion            = $aktion
    }

    try {
        $script:Result | ConvertTo-Json -Depth 2 | Set-Content -Path $GuiConfigPath -Encoding UTF8
        Log "💾 GUI-Konfiguration gespeichert nach $GuiConfigPath" "INFO"
    } catch {
        Log "❌ Fehler beim Speichern der GUI-Konfiguration: $_" "ERROR"
    }

    $form.Close()
})

$btnCancel.Add_Click({
    $script:Result = $null
    $form.Close()
})

# Formular anzeigen
$form.ShowDialog() | Out-Null

# Auswertung
if (-not $script:Result) {
    Write-Host "Abgebrochen durch Benutzer." -ForegroundColor Red
    exit 1
}

# Hauptskript wählen
switch ($script:Result.Aktion) {
    "Report"     { $mainScript = ".\03-Run-Purview-Create-Documentation_GUI_Final_V9.ps1" }
    "AddLanguage" { $mainScript = ".\03-Run-Purview-Add-Language.ps1" }
    "Sortieren"  { $mainScript = ".\03-Run-Purview-Sort-Labels.ps1" }
    default      {
        Write-Error "❌ Keine gültige Aktion ausgewählt!"
        exit 1
    }
}

if (-not (Test-Path $mainScript)) {
    Write-Error "❌ Hauptskript nicht gefunden: $mainScript"
    exit 1
}

$arguments = @(
    "-ExecutionPolicy Bypass"
    "-STA"
    "-File `"$mainScript`""
    "-GuiConfigPath `"$GuiConfigPath`""
) -join ' '

try {
    Log "🚀 Starte Hauptskript im STA-Modus: $mainScript" "INFO"
    Start-Process powershell.exe -ArgumentList $arguments -WindowStyle Normal
    Log "✅ Hauptskript gestartet" "SUCCESS"
} catch {
    Log "❌ Fehler beim Starten des Hauptskripts: $_" "ERROR"
    exit 1
}

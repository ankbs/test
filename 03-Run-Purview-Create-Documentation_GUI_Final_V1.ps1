...
function Start-SplashInThread {
    param (
        ...
        [string]$LogoGIFUrl,
        [string]$ProductLogoBase64 = "",
        [string]$LogoUrl = ""  # 🔧 Ergänzt: URL als Fallback für Produktlogo
    )
    ...
    $ps.AddScript({
        ...
        try {
            ...
            elseif ($CompanyLogoUrl -and $CompanyLogoUrl.StartsWith("http")) {
                ...
                Write-Host "🌐 Firmenlogo von URL geladen: $CompanyLogoUrl"
            }
            elseif ($CompanyLogoBase64 -and $CompanyLogoBase64.Length -gt 100) {
                ...
                $image = [System.Drawing.Image]::FromStream($ms)
                Write-Host "🧬 Firmenlogo aus eingebettetem Base64 geladen."
            }
        ...
        try {
            ...
            if ($ProductLogoBase64 -and $ProductLogoBase64.Length -gt 100) {
                ...
                Write-Host "🧬 Produktlogo aus Base64 geladen."
            }
            elseif ($LogoUrl -and $LogoUrl.StartsWith("http")) {
                $imgBytes = $wc.DownloadData($LogoUrl)
                $ms = New-Object IO.MemoryStream (,[byte[]]$imgBytes)
                $imageProduct = [System.Drawing.Image]::FromStream($ms)
                Write-Host "🌐 Produktlogo von URL geladen: $LogoUrl"
            }
        ...
        # === Ladeanimation oder Fortschrittsbalken
        if (-not $UseProgressBar) {
            try {
                ...
                Write-Host "⚙️ Animiertes GIF geladen: $LogoGIFUrl"
            } catch {
                Write-Host "⚠️ Fehler beim GIF-Download: $_"
            }
        } else {
